<template>
    <Head>
     ...
     <div class="w-full min-h-screen bg-gray-100 relative max-w-6xl">
        <div class="h-56 w-full bg-gradient-to-r from-purple-500 to-green-600">
            <div class="flex flex-row justify-between p-8">
                <div>
                    <div class="relative w-full flex flex-row">
                        ...
                        <input v-model="search" type="text" id="search-dropdown" class="ml-4 block w-full z-20 text-sm text-white placeholder-white bg-transparent focus:ring-white" placeholder="Search Events..." required="">
                    </div>
                </div>
            ...
            <div class="flex flex-row justify-between p-8">
                <div class="flex flex-row justify-between gap-6 text-white">
                    <h2 class="text-4xl font-thin">Events</h2>
                    <svg @click="addNewEvent()" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mt-3 hover:text-gray-500">
        ...
        <div class="mx-8 bg-white relative -top-8 rounded-sm p-8">
            This is where you can add and manage events to be held in store store. Set the date, time, and price of the event, customers will be able to purchase tickets on the online events page. Click on the 
            <svg  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 inline">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg> icon above to add a new event.
    ...
    <table class="w-full text-sm text-left text-gray-500 mt-8">
        ...
        <tbody>
            <tr class="bg-white border-b" v-for="event in events" :key="event.id" :class="event.enabled == false ? 'bg-red-300' : ''">
                ...
                <td class="py-4 px-4">
                    <label :for="event.id+'-toggle'" class="inline-flex relative items-center mr-5 cursor-pointer">
                    <input type="checkbox" v-model="event.enabled" :value="event.enabled" :id="event.id+'-toggle'" class="sr-only peer" @input="changeStatus(event.id)">
                    <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-orange-500"></div>
                    </label>
                </td>
                <td class="py-4 px-4 flex flex-row mt-2">
                    <button @click="editEvent(event)" class="font-medium text-blue-600 hover:text-red-500 mr-2">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                        </svg>
                    </button>
                    <button v-if="!showDeleted" @click="deleteEvent(event)" class="font-medium text-blue-600 hover:text-red-500">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5m6 4.125l2.25 2.25m0 0l2.25 2.25M12 13.875l2.25-2.25M12 13.875l-2.25 2.25M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                        </svg>

                    </button>
                    <button v-else @click="restoreEvent(event)" class="font-medium text-blue-600 hover:text-red-500">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 15l-6 6m0 0l-6-6m6 6V9a6 6 0 0112 0v3" />
                            </svg>

                    </button>
                    <button v-if="showDeleted" @click="destroyEvent(event)" class="font-medium text-blue-600 hover:text-red-500 ml-2">
                         <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <DeleteEvent
         ...
         :event="event"
         @delete-event="submitDeleteEvent"
         @close="close"
        />
...
import DeleteEvent from "../../Forms/Events/DeleteEvent.vue"
...
export default {
    ...
    layout: Admin,
    components: { Message, Notification, EventForm, DeleteEvent, BreadCrumbs, AdminTopNav },
    props: {
        filters: Object,
        errors: Object
    },
     methods: {
        onSearchInput(){
            ...
            ,
             Inertia.get('/dashboard/events', { deleted:this.showDeleted, search: this.search }, {
                 preserveState: true,
                 replace: true
             })
        },
        changeStatus(eventId) {
             Inertia.post('/dashboard/events/status/'+eventId, {
                preserveState: true,
                 replace: true
             })
             setTimeout(() =>
        ...
        submitDeleteEvent(event){
            ...
            if (event.deleted_at){
                Inertia.delete('/dashboard/events/'+event.id+'/destroy')
            } else {
                Inertia.delete('/dashboard/events/'+event.id)
            }
             setTimeout(() =>
            this.showNotification = true
            , 1000);
        ...
        restoreEvent(event){
           Inertia.get('/dashboard/events/restore/'+event.id)
        },
        submitEvent(event){
            ...
            if (event.id){
                Inertia.post('/dashboard/events/'+event.id, event, {
                  preserveState: true,
                    replace: true
                })
            } else {
                Inertia.post('/dashboard/events', event, {
                 preserveState: true,
                    replace: true
                })
            }
